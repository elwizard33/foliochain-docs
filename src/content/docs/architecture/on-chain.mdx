...
<Aside type="tip">
**Smart Contracts Meet Autonomous Agents**: FolioChain's contracts expose specialized interfaces that allow authorized AI agents to call functions directly, enabling true autonomy while maintaining security and compliance.
</Aside>
...
<Aside type="tip">
Every line of FCP's on-chain logic is a covenant with Costa Rican law—not mere guidelines, but hard-coded legal requirements. Now, with agentic architecture, these laws are not just enforced but intelligently optimized by autonomous agents.
</Aside>
